...
fn main() {
    let mut count = 0;
    for seed in 2048..0x20000 {                                                            // number or hexadecimal values here will let you narrow the search's seed range (game supports 2048 through 131072)
        let (normal, _hard) = gen_deep_dive_pair(seed);                                    // _hard indicates Elite dive, normal indicates regular dive

        if _hard.missions[0].biome != EBiome::BIOME_RadioactiveZone {                      // [0] indicates Stage 1 (use 1 or 2 for stages 2 and 3 respectively), EBiome filters biome
            continue;
        }
